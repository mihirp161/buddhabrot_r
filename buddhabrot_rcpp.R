...
buddhabrot <- function(iterations = 1e6, size = 1000, max_iter = 1000) {
  ...
  for (batch in 1:num_batches) {
    ...
    for (res in results) {
      ...
      points_matrix[cbind(x_valid, y_valid)] <- points_matrix[cbind(x_valid, y_valid)] + 1
    }
...
buddhabrot(iterations = 1e6, size = 1000, max_iter = 1500)  # Start with smaller parameters
